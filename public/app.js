...
function renderSlideshow(scenes) {
    ...
    function detectLanguage(text) {
        // Heuristique simple: si beaucoup de mots anglais, en-US, sinon fr-FR
        const englishHints = ['the', 'and', 'was', 'night', 'door', 'dark', 'I'];
        const count = englishHints.reduce((acc, w) => acc + (text.toLowerCase().includes(w) ? 1 : 0), 0);
        return count >= 3 ? 'en-US' : 'fr-FR';
    }

    // Rendu du diaporama dans le canvas
    function renderSlideshow(scenes) {
        const ctx = sceneCanvas.getContext('2d');
        let idx = 0;
        ...
        // Stop previous timers when regenerating
    ...
    // Enregistrer WebM expérimental du canvas avec audio ambiant WebAudio
    async function recordWebM() {
        ...
        // Audio ambiant simple (bruit) pour la piste audio
        const ctx = new (window.AudioContext || window.webkitAudioContext)();
        const dest = ctx.createMediaStreamDestination();
        const osc = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type = 'sawtooth';
        osc.frequency.value = 20; // très basse fréquence pour effet menaçant
        gain.gain.value = 0.02;   // volume faible
        osc.connect(gain);
        gain.connect(dest);
        osc.start();
